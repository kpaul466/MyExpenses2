...
const App: React.FC = () => {
  ...
  const handleAddTransaction = (tx: Transaction) => {
    ...
    if (tx.type === 'EXPENSE' && typeof Notification !== 'undefined' && Notification.permission === 'granted') {
      ...
      if (tx.groupId) {
        ...
        if (group && group.budget > 0) {
          const groupSpent = allTx.filter(t => t.groupId === group.id && t.type === 'EXPENSE' && new Date(t.timestamp).getMonth() === currentMonth).reduce((s, t) => s + t.amount, 0);
  ...
  const handleLoginSuccess = async (token: string, expiresIn: number = 3500) => {
    ...
    try {
      ...
      if (e.message && e.message.includes('401')) {
        ...
        alert('Google Drive API error (403). Please ensure you have granted the necessary permissions.');
      } else {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        ...
        >
          ...
          <h2 className="text-2xl font-black text-slate-800 tracking-tight font-heading mb-2">Connect to Drive</h2>
          <p className="text-sm text-slate-500 mb-6 leading-relaxed">
            A Google Drive connection is required to keep your data safely synced and secure across devices.
          </p>
          <button 
            onClick={login}
          ...
          <div className="bg-amber-50 rounded-2xl p-4 text-left border border-amber-100">
            <div className="flex items-start gap-2 text-amber-800 mb-2">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider">Troubleshooting Login</h3>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50/50 via-white to-rose-50/50 flex flex-col relative font-['Inter']">
        ...
        <main className="flex-1 px-4 overflow-y-auto pt-4 pb-28 relative overflow-x-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeView === 'planner' && <PlannerView categories={categories} transactions={transactions} privacyMode={prefs.expensePrivacy} onAddTransaction={(tx) => { handleAddTransaction(tx); }} />}
              {activeView === 'settings' && (
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
      ...
      <AnimatePresence>
        {activeView === 'add' && <AddExpenseSheet categories={categories} people={people} initialTransaction={editingTransaction} onRefreshCategories={refreshData} onSave={(tx) => { handleAddTransaction(tx); setEditingTransaction(null); setActiveView('dashboard'); }} onCancel={() => { setEditingTransaction(null); setActiveView('dashboard'); }} />}
      </AnimatePresence>
